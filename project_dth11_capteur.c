...
static int my_dth11_probe(struct platform_device *pdev) {
    ...
    of_property_read_u32(pdev->dev.of_node,"id_broche",&dht11_pin); /* id_broche (propriété qui contien */
...
static ssize_t my_dev_read(struct file *flip, char *buf, size_t len, loff_t *off)
{
    ...
    if(*off == 0){
      ...
      sprintf(kbuf,"[Temp:%2d|Hum:%2d]",temperature,humidity);
...
static struct miscdevice my_miscdevice = {
        ...
        .name = "project_dth11", /*"my_dht11",*/ /* le nom dans /dev/ */
        .mode = 0666,
};
